...
""") + self.SH_DEFAULT
    ...
    def book_info(self, info):
        if APIVER == 1:
            ...
            description = description.get("text/plain", None) or "n/d"
        for t in [
...
class SafariBooks:
    ...
    def __init__(self, args):
        ...
        global APIVER, MODERATE_LEN, MODERATE
        ...
        if APIVER == 2:
            for c in self.book_chapters:
                if "cover" in self.get_filename(c):
                    ...
                    if len(c_images):
                        self.book_info["cover"] = c_images[0]
                    break
    ...
    def check_login(self):
        ...
        if response == 0:
            ...
            self.display.exit(
                "Authentication issue: unable to access profile page (status = %s): %s" %
                (response.status_code, PROFILE_URL)
            )
    ...
    def get_book_info(self):
        ...
        if not response.text.strip():
            self.display.exit(
                "API: empty response when retrieving book info.\n"
                "    Your session cookies may be expired. Please update `cookies.json` and try again."
            )
        ...
        try:
            ...
            self.display.exit(
                "API: unable to parse book info (status %s).\n    Response: %s" %
        ...
        if APIVER == 2:
            # Supplement v2 response with authors/publishers/etc from v1 (best-effort; v1 may be unavailable)
            try:
                response2 = self.requests_provider(self.APIV1_INFO.format(self.book_id))
                if response2 != 0 and response2.text.strip():
                    response2 = response2.json()
                    for k in ["authors", "subjects", "topics", "rights", "publishers", "web_url"]:
                        if k in response2:
                            response[k] = response2[k]
            except (ValueError, Exception):
                self.display.info("Note: v1 API unavailable; author/publisher metadata may be missing.")

        # Ensure web_url fallback so later code doesn't crash
        if "web_url" not in response or not response["web_url"]:
            response["web_url"] = SAFARI_BASE_URL + "/library/view/-/" + self.book_id + "/"
    ...
    def get_book_chapters(self, chapter_url):
        ...
        if not response.text.strip():
            self.display.exit(
                "API: empty response when retrieving book chapters.\n"
                "    Your session cookies may be expired. Please update `cookies.json` and try again."
            )
        ...
        try:
            ...
            self.display.exit(
                "API: unable to parse book chapters (status %s).\n    Response: %s" %
                (response.status_code, response.text[:500])
            )
        ...
        for c in result:
            self.display.log(
                "Moving chapter %s with URN %s to the front of the current list" %
                (response["results"].index(c), c[urntype])
            )
        ...
        for ci, c in enumerate(result):
            if "toc" in self.get_filename(c) or c["title"] == "nav":
                if c.get("indexed_position", None) is None:
                    toc_index = ci
                break
        if toc_index >= 0:
            self.display.log(
                "Moving chapter %s with URN %s to the second place of the current list" %
                (toc_index, c[urntype])
            )
    ...
    def is_image_link(url: str):
        return any(x in url for x in ["cover", "images", "graphics"]) or \
               (pathlib.Path(url).suffix[1:].lower() in ["jpg", "jpeg", "png", "gif"])
            ...
            if not self.url_is_absolute(link):
                if not self.is_doc_link(link) and self.is_image_link(link):
                    ...
                    if image_path:
                        new_link += image_path + "/"
                    new_link += image_name
                    full_link = functools.reduce(
                        lambda a, b: a + b[self.str_overlap(a, b):],
                        [self.book_info["url"], "/files/", link]
                    )
                ...
                if self.book_info.get("url", "") and self.book_info["url"] in link:
    ...
    def parse_html(self, root, first_page=False):
        ...
        if self.args.theme != 'none':
            page_css += "<link href=\"Styles/%s\" rel=\"stylesheet\" type=\"text/css\" />" % SB_THEME_FILE
            src_sb_css = pathlib.Path(PATH) / pathlib.Path(SB_THEME_FILE)
            if src_sb_css.exists():
        ...
        for img in tsoup.find_all('img'):
            ...
            if img_style and ('width' in img_style or 'height' in img_style):
                ...
                if img_width or img_height:
                    fixed_n += 1
        if fixed_n > 0:
            self.display.log("[fix_overconstrained_images] img tags changed: %s" % fixed_n)
        return str(tsoup)
    ...
    def save_page_html(self, contents):
        ...
        html_bytes = self.BASE_HTML.format(contents[0], contents[1], theme_mode).encode("utf-8", 'xmlcharrefreplace')
        with open(os.path.join(self.BOOK_PATH, "OEBPS", self.filename), "w") as html_file:
            html_file.write(self.fix_overconstrained_images(html_bytes))
    ...
    def local_image_path(self, full_url):
        if APIVER == 1:
            ...
            rel_url = full_url.split(baseurl)[-1].lstrip("/")
            local_path = rel_url
            for imd in image_dirs:
                if local_path[0:len(imd)] == imd:
                    local_path = local_path[len(imd):]
                    break
    ...
    def _thread_download_font(self, font_info):
        ...
        font_file = (pathlib.Path(self.css_path) / font_info[1]).resolve()
    ...
    def create_toc(self, toc_url):
        ...
        if not response.text.strip():
            self.display.exit(
                "API: empty response when retrieving TOC.\n"
                "    Your session cookies may be expired. Please update `cookies.json` and try again."
            )
        ...
        try:
            ...
            self.display.exit(
                "API: unable to parse TOC (status %s).\n    Response: %s" %
                (response.status_code, response.text[:500])
            )
...
if __name__ == "__main__":
    ...
    arguments.add_argument(
        "--api", metavar="<API>", default=2, type=int,
        help="Choose the API version (1 or 2, default is 2)."
    )
    arguments.add_argument(
        "--delay", metavar="<DELAY>", default=0.3, type=float,
        help="Seconds to wait between requests (default 0.3; set 0 to disable)."
    )
    arguments.add_argument(
        "--theme", metavar="<THEME>", default='none',
        help="Styling theme: 'black', 'white', 'sepia', or 'none' (default)."
    )
    arguments.add_argument("--help", action="help", default=argparse.SUPPRESS, help='Show this help message.')
    arguments.add_argument(
        "bookid", metavar='<BOOK ID>',
        help="Book digits ID that you can find in the URL:"
             " `" + SAFARI_BASE_URL + "/library/view/book-name/XXXXXXXXXXXXX/`"
    )
